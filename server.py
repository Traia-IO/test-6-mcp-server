#!/usr/bin/env python3
"""
Test 6  MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('test-6_mcp')
...
logger.info("="*80)
logger.info(f"Test 6  MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://petstore.swagger.io/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Test 6  MCP Server", host="0.0.0.0")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="test-6-mcp-server"  # MCP server ID for tracking
    )
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Test 6  MCP Server")
    logger.info("="*80)
